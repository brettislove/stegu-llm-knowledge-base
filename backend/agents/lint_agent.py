...
if anything, to fix (likely via a normal ingest/distill run, or by hand).
...
1. ORPHAN PAGES — pages that exist as files under a category folder but
   have no corresponding entry in index.md.
2. BROKEN REFERENCES — entries in index.md (or explicit page-to-page
   mentions you find via grep) that point to a file which doesn't exist,
   e.g. a renamed or deleted slug.
...
5. CONTRADICTIONS — two or more pages asserting conflicting facts about the
   same subject (e.g. two different prices for the same product, or one
   page saying a product is active while another implies it's discontinued).
   This is the check that most needs real judgment, not just file listing —
   take your time on it.

Process:
1. Read index.md first for an overview of what should exist.
2. Use list_files to see what actually exists in each category folder, and
   compare against index.md to find orphans/broken references.
...
4. Write your findings to lint-report.md using write_file — this OVERWRITES
   ...
   Structure it with a heading per check category, and under each, either
   "No issues found" or a bullet list of specific pages/problems.
...
def run_lint() -> dict:
    ...
    user_message = "Run a full lint pass over the wiki now and write the report."
